-- ===================================
-- Final Database Update Script - CLEAN VERSION
-- Karam Platform Database Enhancements
-- نسخة نظيفة وخالية من الأخطاء
-- ===================================
...
-- ═══════════════════════════════════
-- Part 3: Discount Code Functions
-- ═══════════════════════════════════
...
BEGIN
    ...
    IF v_discount_record.usage_limit IS NOT NULL AND 
       v_discount_record.times_used >= v_discount_record.usage_limit THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::VARCHAR, NULL::DECIMAL,
                     0::DECIMAL, 'Code usage limit reached';
        RETURN;
    END IF;

    IF p_booking_amount < v_discount_record.min_booking_amount THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::VARCHAR, NULL::DECIMAL,
                     0::DECIMAL, 'Minimum amount not met';
        RETURN;
    END IF;
    ...
    IF jsonb_array_length(v_discount_record.applicable_families) > 0 AND
       NOT (v_discount_record.applicable_families @> jsonb_build_array(p_family_id::text)) THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::VARCHAR, NULL::DECIMAL,
                     0::DECIMAL, 'Code not applicable to this family';
        RETURN;
    END IF;
...
-- ═══════════════════════════════════
-- Part 4: Booking Time Slots Table
-- ═══════════════════════════════════
...
DROP POLICY IF EXISTS "Users can view their booking time slots" ON public.booking_time_slots;
...
DROP POLICY IF EXISTS "Admins can manage booking time slots" ON public.booking_time_slots;
...
-- ═══════════════════════════════════
-- Part 5: Concurrent Booking Functions
-- ═══════════════════════════════════
...
BEGIN
    ...
    IF v_total_capacity IS NULL THEN
        RETURN QUERY SELECT FALSE, 0, 0, 0, 'Family not available'::TEXT;
        RETURN;
    END IF;
    ...
    IF v_available_capacity >= p_guest_count THEN
        RETURN QUERY SELECT 
            ...
            ('Available: ' || v_available_capacity::TEXT || ' of ' || v_total_capacity::TEXT)::TEXT;
    ELSE
        RETURN QUERY SELECT 
            FALSE, 
            v_total_capacity, 
            v_booked_capacity, 
            v_available_capacity,
            ('Not enough: ' || v_available_capacity::TEXT || ' available, ' || p_guest_count::TEXT || ' required')::TEXT;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ═══════════════════════════════════
-- Part 6: Add is_active Column
-- ═══════════════════════════════════
